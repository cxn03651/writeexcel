...
def create_doc_summary_property_set(properties)
  ...
  (0 .. @properties -1).each do |i|
    property_offsets_0 = property_offsets_0 +
                                  [properties[i][0], offsets[i]].pack('VV')
  end
...
def pack_VT_LPSTR (string, codepage)
  ...
  return data
   ...
   def pack_VT_FILETIME(localtime)
       ...
       seconds = Time.at(localtime).to_i

       # Add the number of seconds between the 1601 and 1970 epochs.
       seconds += 11644473600

       # The FILETIME seconds are in units of 100 nanoseconds.
       nanoseconds = seconds * 1.0e7

       # Pack the total nanoseconds into 64 bits.
       time_hi = Integer(nanoseconds / 2**32)
       time_lo = 0    # POSIX::fmod(nanoseconds, 2**32)

       data = [type, time_lo, time_hi].pack('VVV')
